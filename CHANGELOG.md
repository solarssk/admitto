...
- Repeated failed sign-in attempts on an admin account, use of the emergency two-factor bypass, and changes to organisation login/security settings (SSO provider or Cloudflare Access) now actually trigger the alert configured in Organisation Settings → Notifications, on top of the audit trail these already wrote before. Admin login from a country not seen before on that account is still configured but not yet wired up - that alert type doesn't fire yet.
