...
- An event item's "Attendee data field" picker (Requirements page) no longer lets the same custom field be selected on more than one item. Nothing previously stopped it, so the same hint (for example "Shirt size: L") could silently render on two different items' check-in cards for no reason. The picker now grays out a field already claimed by another item, with a tooltip naming which one, and saving a config that references a field another item already has is rejected server-side as a defense-in-depth backstop against concurrent edits - every server-side write that assigns a field now consistently runs under the same serializable-and-retry protocol, closing a gap where an ordinary create running concurrently with a disabling/badge-affecting edit could each end up believing the field was still free.
