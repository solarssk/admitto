...
### Added
- Bulk **Send tickets** on the attendees list: `POST .../attendees/bulk-resend` with `target` `unsent` (default) or `all` (max 500 per request); confirmation modal in admin SPA; audit via `mail_bulk_resend` in attendee action log
