...
- Event Settings changes to an event's name, date, timezone, or event hours are now pushed in the background to every already-issued wallet pass for that event, so passes stay in sync without an explicit reissue.
