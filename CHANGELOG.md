...
- Check-in: duplicate-scan debounce and buffer-clear-on-accept are now measured at the moment a scan is accepted, not once it reaches the front of the FIFO queue — a slow first request could otherwise let a genuine duplicate through, or leave stale text for a later scan's keystrokes to land on. Auto-advance also no longer clears an unrelated, still-in-progress scan's buffer, and Undo now queues behind an in-flight scan instead of racing it — the backend rolls back whichever check-in is latest for the device, so a concurrent admit could otherwise get undone instead of the one shown on screen (#277 review follow-up).
