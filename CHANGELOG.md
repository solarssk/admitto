...
- Attendees' header "More actions" has a new "Refresh status" item (once the event has Wallet configured) that pulls the current device-registration status from the provider for every wallet pass across the whole event at once, including a voided one - runs in the background and reports a summary toast once it finishes, the same way the header's "Push updates" does.
