...
- Check-in scan bar: the hint under the field no longer says "Keyboard wedge auto-submits" — an event-day operator reading that had no idea what a "keyboard wedge" is (PO review). It now describes what actually happens in plain language: **"Scan a code — it submits itself · type a name, then press Enter · Esc clears the field."** A pasted or manually-typed ticket token now actually works when the operator does what the hint says (press Enter) — previously, an explicit Enter on a long value additionally required proof it arrived as a genuine hardware-scanner burst, so a pasted or slowly-typed real code fell through to a name/email lookup instead and failed with a misleading "No attendees matched that search" (bot review; verified against a real token decrypted from the dev DB). The silent auto-submit that fires with no Enter at all still requires a genuine burst, unchanged — that path has no user confirmation, so it stays the strict one. That first fix over-corrected, though: it made *any* long value on Enter attempt a scan, so a genuinely long name/email search (which #262 originally protected) started failing the same way a real token used to. A real ticket token is a random byte string that never contains a space or `@` — an explicit Enter now only attempts a scan when the value is long *and* doesn't look like a name/email; anything with a space or `@` still searches by name/email regardless of length (bot review, round 4; verified live: a pasted 34-character email now correctly finds the attendee, and a pasted token still completes the check-in). Round 5 (self-review after the above): that "doesn't look like a name/email" check was still a loose exclusion (no space/`@`), not a real shape check — it let an unbounded-length pasted URL, UUID list, or other space-free text reach the scan endpoint, and still misrouted a genuinely long single-word name or company. Replaced with `looksLikeInternalToken` (`@admitto/tickets`) — the same precise, length-bounded base64url check the server itself uses to recognize a raw token — closing both gaps in one fix (verified live: a pasted URL now correctly reports "No attendees matched" via lookup instead of "invalid code" via a scan attempt).
