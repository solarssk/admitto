...
- My account page (#324): Profile section redesigned — 2-column grid on desktop (editable fields left: display name + regional format; read-only right: email, sign-in method, roles); `<select>` replaced with `Select` from `@admitto/ui`; "Date format" label renamed to "Regional format" with updated hint; new read-only "Sign-in" row shows `Local password`, `Identity provider (SSO)`, or `Local password + Identity provider` based on `has_local_password` and `roles[].is_oidc`; 2FA status badge and action button are now on a single compact row; the "Set up authenticator" enrollment view (QR code, backup codes, confirmation) now lays out QR + backup codes side-by-side with a compact code-input/confirm row below, instead of one tall vertical stack, so enrolling no longer grows the "Password & two-factor authentication" card taller than the Password column or triggers scrolling — Password's natural height is the fixed reference for the card. Active sessions table scrolls within the card at `max-height: 380px` with sticky header.
