...
- Requirements → Custom attendee fields: empty registry uses EmptyState (no empty table row), the table shows Description instead of Type, and the type icon has a hover tooltip. Field-type buttons in the edit modal share equal width with clearer icon/label spacing; type hint uses standard `.at-hint` size and Options sits in the same field group. Event items and custom-fields table rows share the same cell padding.
