...
- The instance now publishes a `robots.txt` that disallows crawling, and every public page (ticket, QR code, event map, branding assets, staff sign-in) sends `X-Robots-Tag: noindex, nofollow`, so search engines and AI crawlers that respect these signals no longer index attendee ticket links or scan QR codes into image search.
