...
- Security alerts from Organisation Settings → Notifications (repeated failed logins, MFA break-glass use, settings changed, new-country logins) named the affected account with a masked, partially garbled identifier (for example "a*** at example.com") instead of a real name or email - the account is now shown by its display name where known, otherwise its real email. Settings-change alerts also now name the specific thing that changed (for example an SSO provider's own configured name) instead of only its internal database id, which had no way to be looked up from the admin UI.
