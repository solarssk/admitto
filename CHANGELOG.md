...
- `/.well-known/change-password` now redirects to the sign-in change-password page, following the W3C "Well-Known URL for Changing Passwords" convention Safari, Chrome, and several password managers already look for - so a browser or password manager's own "change password" shortcut lands staff directly on the right page instead of guessing.
