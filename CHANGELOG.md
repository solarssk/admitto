...
- Check-in scan bar: the hint under the field no longer says "Keyboard wedge auto-submits" — an event-day operator reading that had no idea what a "keyboard wedge" is (PO review). It now describes what actually happens in plain language: **"Scan a code — it submits itself · type a name, then press Enter · Esc clears the field."** (An earlier version of this copy also said "or paste a code", but a pasted token is deliberately never treated as a scan — a paste event, or any single change with more than one new character, marks the input as manual typing rather than a scanner burst, so it doesn't auto-submit and Enter routes it to a name/email lookup instead, which fails; bot review.)
