...
- Identity providers migration cleanup (#266 slice 5): the legacy server-rendered identity admin is removed — `/admin/auth/providers*` and `/admin/auth/cf-access*` HTML routes, their handlers (`auth-providers-routes.ts`, `cf-access-routes.ts`), the HTML renderers (`auth-providers-html.ts`, `cf-access-html.ts`), and the `renderAdminShell` sidebar + `SETTINGS_SUBNAV_ITEMS` block are deleted. The SPA at `/admin/settings/identity/*` is now the only identity admin surface; the JSON API (`/api/admin/identity/*`, slice 1) is unaffected. The `cf-access-routes` integration test was retargeted from the deleted HTML routes to the `/admin` SPA shell (and `/api/admin/identity/providers` for the CF no-role 403 message). Settings page now persists the active in-page tab in the URL (`?tab=general|mail|security|archiving`, replace) so Back from the Identity sub-section restores the tab instead of resetting to General (closes the #296 TODO); `?tab=identity` still redirects to the canonical Identity route. Docs/env examples updated to reference Settings → Identity instead of the legacy paths; the stale `/admin/auth` Vite dev-proxy rule was removed.
