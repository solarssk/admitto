...
- The rest of the admin team is now notified (via Organisation Settings → Notifications, same as the other security alerts) whenever a user is granted the admin or superadmin role, matching NIST SP 800-53 AC-2(1) - previously a role change of this kind left no alert at all, only the existing audit trail. A same-or-lower-rank role change, such as moving a superadmin down to admin, is a demotion and correctly does not trigger this alert.
