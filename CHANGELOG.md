...
- An attendee's Wallet card now shows a "Device" row (browser and OS, e.g. "Mobile Safari / iOS") once their wallet pass has a confirmed registration - captured on Admitto's own redirect, the same hop that already issues and refreshes the pass, so it fills in on the next click even for a pass added before this existed. The row only appears once the pass is genuinely confirmed installed, so a corporate mail security scanner pre-fetching the wallet link can never show up as "the" device.
