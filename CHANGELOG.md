...
- Event cards now keep the last forecast after the event is over, shown as **Last forecast** (for example 18°C) with the weather provider's credit. Weather providers cannot look back, so Admitto now saves the latest forecast it sees for an event's day whenever someone opens the Events list, the event, or the check-in event list while the day is still ahead, and shows that one afterwards. Events that ended before this release, or whose day nobody had a list open for in the days before, have none and show no weather chip. Moving an event to another day or place discards its saved forecast. Includes a database migration (one new nullable column on events), applied by the usual migration step.

### Changed

- Admitto now notices when PassCreator has voided or expired a wallet pass on its own. The background wallet sync and **Refresh status** read the pass again and, when PassCreator reports it voided or expired, mark it **Voided** here too, so it stops looking active and offers the voided actions (Restore, Delete). Until a PassCreator time zone setting arrives in a following release, an expired pass is shown as voided rather than expired. The wallet sync and **Refresh status** only read active passes of events that are not archived from now on: a voided or expired pass keeps the last registration counts it had and is no longer polled, which also saves PassCreator API calls, and **Refresh status** is no longer offered for one.
- PassCreator's voided notification is now treated as a signal to read the pass again instead of being taken at face value. A late redelivery can no longer void a pass that was restored in the meantime, and if the read fails the notification is answered with an error so PassCreator sends it again. After a **Void** or **Restore** made in Admitto, a stale answer from PassCreator within the next ten minutes is ignored rather than undoing it.
- **Refresh status** (single attendee, a selection, and the whole event) now also works on an archived event and when the event's Wallet switch is off, because it only reads from the provider. Void, Push updates and Delete stay blocked there as before.
...
- Security notification emails now show readable context instead of internal identifiers. For example, an administrator-role alert names the person who made the change and the person receiving the role in the message, then shows **Role: Administrator** and **Scope: Organisation name**; it never exposes `actor_user_id`, `target_user_id`, or `scope_id`. The other alerts likewise use reader-facing labels for failed-attempt counts, emergency actions, sign-in location/device/time, and identity-provider changes. Future notification types show no email details until their human-facing labels are defined.
...
- Event cards no longer show a false "Weather unavailable" error once the event is over. Before, every event whose day had passed showed a cloud-off chip saying the forecast could not be loaded and pointing to External services, even though the weather provider worked fine, because weather providers only serve today and the days ahead. Weather for events that are still ahead, and the public ticket page, are unchanged.
...
### Security
- Cloudflare Access staff sign-ins now bind only to an existing direct OIDC identity through a forwarded immutable subject, reconcile that provider's managed group roles on every edge sign-in, and fail closed when a required role claim is unavailable. Enabling it requires selecting an enabled direct identity provider.
