...
### Added
- **Requirements — Custom attendee fields registry (part of #372).** Event-wide attendee data
  fields (dietary requirements, parking, or anything else that isn't tied to a specific
  hand-out item) now have their own management card on the Requirements page — define a field's
  label, type (text/select/boolean), required flag, and options once, independent of any event
  item. Event items reference these fields to show one as an operator hint at check-in (e.g. a
  gift bag's shirt size) instead of each item separately redeclaring the same field with its own
  metadata, which is what let two items disagree about the same field before. The check-in
  operator card now reads a field's label from this registry. A follow-up will wire the attendee
  edit form, CSV/XLSX import, and export columns onto the same registry — for now those still
  read from the previous per-item mechanism, unaffected by this change. A field can only be
  deleted from the Requirements card, and is blocked (409) while any item still shows it as a
  hint.
