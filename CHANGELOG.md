...
- Check-in scan bar: the hint under the field no longer says "Keyboard wedge auto-submits" — an event-day operator reading that had no idea what a "keyboard wedge" is (PO review). It now describes what actually happens in plain language: **"Scan or paste a code — it submits itself · type a name, then press Enter · Esc clears the field."**
- Check-in — desktop camera no-match: when a scanned QR matches no attendee, the camera view now shows the **same lightweight status card the typed-search path already uses** (`ScanFeedback`), instead of a plain dark result panel borrowed from the mobile overlay that had no color treatment on desktop and read as an unpolished, orphaned leftover (PO review). The full-color result panel is still used on the mobile camera overlay, where it belongs.
