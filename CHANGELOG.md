...
### Changed
- Living ops and security docs now match the current stack: generic **Upgrading** (manual backup first; Redis password note for very old installs), no automatic pre-migration dump / root migrate, product retention and bounce on the **worker** (not app startup or a retention sidecar), and package README CLI notes aligned with workspace builds.
