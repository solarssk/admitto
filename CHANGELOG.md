...
- Event Settings → Wallet's Samsung Wallet switch is now a real, saveable setting instead of a permanently disabled placeholder, and Reports' Wallet platform breakdown now shows or hides its Samsung slice to match - same as the existing Apple/Google toggles. PassCreator has no Samsung Wallet API yet, so no attendee gets an actual Samsung pass either way; this only prepares the toggle and the report for when that support lands.
