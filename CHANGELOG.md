...
### Changed
- Overview page: "Quick actions" link grid (duplicate of sidebar nav) replaced with two information cards — Email delivery breakdown (Sent/Pending/Failed bars with counts) and Recent check-ins feed (attendee name, ticket type, time); check-in events arrive live via the existing SSE subscription (#276).
