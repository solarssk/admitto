...
- **Requirements — Custom attendee fields:** A new read-only "Custom attendee fields" section on the Requirements page aggregates all operator-hint fields configured across event items. Shows Display label, Source field key, and which item the field originates from. No additional DB table — derived from existing EventItem configs (#372).
