...
- Attendee detail's wallet "More actions" menu has a new "Refresh status" action - pulls the pass's current device-registration status directly from the provider on demand, instead of waiting for the periodic background sync to reach that one attendee (which can take up to 30 minutes, longer if many other passes are stale system-wide). For when the provider's own dashboard already shows a pass as added but Admitto hasn't caught up yet. Shares the same rate limit as the other single-attendee wallet actions (void/restore/reissue/delete), since it also calls the provider once per request.
