...
- An SMTP send failure that doesn't come back as a recognizable temporary or permanent error (for example, the mail server dropping the connection mid-send under load instead of replying with a clean error code) is now retried by the mail delivery worker the same as any other temporary failure, instead of being marked permanently failed after a single attempt.
