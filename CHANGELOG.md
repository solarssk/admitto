...
- Settings page: the active in-page tab is persisted in the URL (`?tab=general|mail|security|archiving`, merged via `replace` so tab clicks don't stack history or wipe unrelated params) — Back from the Identity sub-section now restores the operator's tab instead of resetting to General (closes the #296 TODO); `?tab=identity` still redirects to the canonical Identity route. Legacy `/admin/auth/providers*` and `/admin/auth/cf-access*` URLs redirect (SPA-side) to `/admin/settings/identity/*` so old bookmarks/docs links land on the identity admin instead of a blank outlet (#266 slice 5).
