...
- **My account: passkeys and security keys.** Staff can now register passkeys (Face ID, Windows Hello, and similar) and security keys (for example a YubiKey) as extra two-factor sign-in methods alongside an authenticator app. Each one is given a name so multiple keys can be told apart, and there is no limit on how many can be registered. Setting up, resetting, or adding a two-factor method now opens in a popup instead of expanding inline on the page.
