...
- Notes: flagged by `scripts/check-licenses.mjs` (see [SECURITY.md](SECURITY.md)'s control table); the gate stays `continue-on-error: true` for this package until it is either confirmed permissive by some other means or the `exceljs`/`unzipper` dependency chain that pulls it in is replaced.
