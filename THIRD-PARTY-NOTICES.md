...
- Notes: Admitto sets `ILA_LICENSE_KEY=redist` and `ILA_FIELDS=country,city` (see `apps/web/.env.example` and the Dockerfile), selecting the MaxMind GeoLite2 City database, fetched from the [node-geolite2-redist](https://github.com/sapics/node-geolite2-redist) community mirror rather than MaxMind's own servers directly - no MaxMind account/license key is needed for this project or its self-hosters. The library code itself is MIT (`node_modules/ip-location-api/LICENSE`); the GeoLite2 data is under MaxMind's [GeoLite2 End User License Agreement](https://www.maxmind.com/en/geolite2/eula) (bundled at `node_modules/ip-location-api/EULA`), which incorporates the Creative Commons Attribution-ShareAlike 4.0 International License and requires attribution: "This product includes GeoLite2 Data created by MaxMind, available from https://www.maxmind.com/." The data is free of charge (EULA §9) with no per-lookup query limit for this offline, self-hosted usage. A self-hoster who instead sets `ILA_IP_LOCATION_DB` (see `deploy/ENV.md`) opts back into the country-only, CC0-1.0/PDDL `ip-location-db` "user" dataset this project previously used by default, avoiding the GeoLite2 attribution requirement in exchange for losing city-level detail.
