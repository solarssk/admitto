 ...
 *
 * A fingerprint is `{ ruleId, filePath, message }` - deliberately EXCLUDING line/column, so a
 * line shifting elsewhere in a file (e.g. someone adds a blank line above an untouched warning)
 * never produces a false "new warning". Only ESLint messages with severity 1 (warning) are
 * fingerprinted; severity 2 (error) already fails `npm run lint`'s own exit code and needs no
 * separate gate here.
 *
...
/** Converts raw ESLint results into warning-only fingerprints (severity 1, no line/column). */
function toFingerprints(results) {
  const fingerprints = [];
  for (const result of results) {
    const relPath = relative(REPO_ROOT, result.filePath).split(sep).join("/");
    for (const msg of result.messages) {
      if (msg.severity !== 1) continue; // warnings only - errors already fail `npm run lint`.
      fingerprints.push({ ruleId: msg.ruleId, filePath: relPath, message: msg.message });
    }
  }
  return fingerprints;
}
...
function fingerprintKey(fp) {
  return JSON.stringify([fp.ruleId, fp.filePath, fp.message]);
}
...
async function check() {
  ...
  // Multiset (count-based) comparison rather than a plain Set difference: since line/column are
  // excluded from the fingerprint, two genuinely distinct warnings in the same file can share an
  // identical fingerprint (same rule, same message text). Counting occurrences means a THIRD
  // occurrence of a signature the baseline only saw twice is still correctly caught as new.
