...
| `WALLET_REFRESH_STATUS_JOB_STALE_RUNNING_MS` | optional | worker | none | no | Fail wallet_refresh_status AdminJobs stuck in running after this many ms (default 30 minutes). |
...
| `CHECKIN_STREAM_MAX_CONCURRENT_PER_EVENT` | optional | app | none | no | Check-in live-updates stream: max streams one operator can hold open at once for one event (for example Check-in, Overview and Reports tabs). Default 3. A different mechanism from the request rate limit: it counts open connections, not connect attempts. Counted per app replica (in process memory), so with several replicas the effective ceiling is this value times the number of replicas. Positive integer; anything else falls back to the default with a boot warning. |
| `CHECKIN_STREAM_MAX_CONCURRENT_PER_ACTOR` | optional | app | none | no | Check-in live-updates stream: max streams one operator can hold open at once across all events. Default 12. Counted per app replica (in process memory), so with several replicas the effective ceiling is this value times the number of replicas. Positive integer; anything else falls back to the default with a boot warning. |
...
_Last generated from 116 distinct keys seen in scan (tests excluded)._
