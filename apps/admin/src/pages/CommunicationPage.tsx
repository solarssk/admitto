...
// Lazy: pulls in CodeMirror plus its language/lint/autocomplete extensions, which pushed this
// page's own chunk over Vite's 500kB warning threshold even though most visits never open the
// Templates tab at all (see issue #1331). Only actually imported the first time that tab renders.
const TemplateEditorCard = lazy(() =>
  import("../communication/TemplateEditorCard.js").then((m) => ({ default: m.TemplateEditorCard })),
);

export type ActiveField = "subject" | "body";
...
export const HIDDEN_PLACEHOLDERS = new Set(["header_image_url"]);
...
export const WALLET_PLACEHOLDERS = new Set(["apple_wallet_url", "google_wallet_url"]);
...
export function placeholderDescription(name: string, isImage: boolean): string {
...
/** Also used by TemplateEditorCard.tsx (lazy-loaded, see the `lazy(...)` import above) to build
 * its own illustrative sample QR chip preview - exported so that code doesn't need its own copy. */
export function svgDataUri(inner: string): string {
  return (
...
/** Swapped into the rendered *email* preview (not the Templates tab's chip picker) in place of the
