 ...
 * on a platform the event currently offers (apple/google/samsung_active_registrations > 0) - the
 * only signal that distinguishes a genuinely installed pass from one that was merely created
...
export function isWalletPassInstalled(
  ...
  platforms: { apple: boolean; google: boolean; samsung: boolean },
): boolean {
  return (
    (platforms.apple && (pass.apple_active_registrations ?? 0) > 0) ||
    (platforms.google && (pass.google_active_registrations ?? 0) > 0) ||
    (platforms.samsung && (pass.samsung_active_registrations ?? 0) > 0)
  );
}
