...
function dualApp(getEventId: (c: import("hono").Context) => string | undefined) {
  const app = new Hono();
...
describe("createCheckinDualAuth — session matrix", () => {
  const app = dualApp(eventIdFromHistoryQuery);

  it("operator matching event → 200", async () => {
    const cookie = await sessionCookieFor(USER_OP_A);
    const res = await app.request(`/api/checkin/test?eventId=${EVENT_A}`, {
  ...
  it("operator wrong event → 401", async () => {
    const cookie = await sessionCookieFor(USER_OP_A);
    const res = await app.request(`/api/checkin/test?eventId=${EVENT_B}`, {
      headers: { Cookie: cookie },
    });
    expect(res.status).toBe(401);
  });
  ...
  it("missing eventId → 400", async () => {
    const cookie = await sessionCookieFor(USER_SUPER);
    const res = await app.request("/api/checkin/test", { headers: { Cookie: cookie } });
