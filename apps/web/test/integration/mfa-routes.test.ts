...
/** Resets the admin's auth-lab state, registers a confirmed WebAuthn credential for them, and
 * logs in with password - the shared starting point for the step-up/verify tests below, which
 * only differ in what they do with the resulting loginRes. */
async function loginAsWebauthnConfirmedAdmin(): Promise<Response> {
...
describe("HTML /mfa/verify — passkey button", () => {
  it("shows the passkey button when the account has a confirmed WebAuthn credential", async () => {
    const loginRes = await loginAsWebauthnConfirmedAdmin();
...
describe("HTML MFA enroll - WebAuthn method choice", () => {
  ...
  it("completes enrollment end to end via a registered passkey: creates the credential, stashes backup codes, and reaches a full session", async () => {
    ...
    // register/finish already promoted the session to backup-codes-required, rotating the
    // token - the pre-promotion login cookie no longer validates from here on.
    const backupCodesRes = await app.request("/mfa/enroll/backup-codes", {
      headers: { ...sameOrigin, ...cookieHeader(finishRes) },
    });
    expect(backupCodesRes.status).toBe(200);
...
describe("POST /api/auth/mfa/remember-device", () => {
  async function loginToFullSession(): Promise<Response> {
    ...
    // login cookie no longer validates - verify's own response carries the new one.
    return verifyRes;
  }
