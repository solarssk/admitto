...
/** Resolve staff principal from a validated Cloudflare Access JWT payload. */
async function resolveCfAccessAuth(
  token: string,
...
/** Resolve staff principal from the session cookie. */
async function resolveSessionCookieAuth(
  c: Context,
  prisma: PrismaClient,
): Promise<StaffAuthResult> {
...
export async function resolveStaffAuthFromRequest(
  ...
  if (config.enabled) {
    ...
    if (token) {
      return resolveCfAccessAuth(token, config, prisma, path);
    }
  }

  return resolveSessionCookieAuth(c, prisma);
}
