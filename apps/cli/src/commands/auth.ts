...
export async function runAuthResetMfa(db: PrismaClient): Promise<void> {
  ...
  await logMfaBreakGlass(db, { action: "reset_mfa", email, userId, quiet: true });
...
export async function runAuthGenerateEmergencyRecovery(db: PrismaClient): Promise<void> {
  ...
  await logMfaBreakGlass(db, { action: "generate_emergency_recovery", email, userId, quiet: true });
