...
| **v0.6.x** ✅ | Delivered - passkeys/security keys usable for sign-in and step-up (not just registration), "Forget all trusted devices", session-cookie and step-up security hardening, first-time 2FA method choice; later patches added Reports' Wallets and Custom fields tabs, a Samsung Wallet settings toggle, passkey autofill (conditional mediation) on the login page, and multi-arch container images |
