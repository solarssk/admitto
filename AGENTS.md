...
**Root cause, confirmed via the GitHub checks API, not guessed:** this repo runs SonarCloud analysis **twice** on every PR - the CI-based `sonarcloud` job (`ci.yml`, `SonarSource/sonarqube-scan-action`, app `GitHub Actions`) *and* the SonarCloud GitHub App's own **Automatic Analysis** (posts as a separate check named "SonarCloud Code Analysis", app `SonarQubeCloud`) - both completed successfully on the same commit seconds apart. `docs/dev/sonarcloud-ci-coverage-migration.md` (written for the CI migration, PR #1245) explicitly required, as step 3, turning Automatic Analysis **off** in SonarCloud's `Administration > Analysis Method` page after adding the CI-based scan - quoting SonarSource's own docs: "If you enable automatic analysis, you must ensure that you do not have any CI-based analyses process" - and that step was apparently never actually done. Which of the two conflicting analyses' results the PR issue list/dashboard reflects at any given moment isn't something a code change controls, so this - not an Automatic-Analysis-vs-scanner rule-engine gap - is the most likely reason a rule fix confirmed live on SonarSource's side doesn't consistently show up here. Fixing this needs a human with SonarCloud dashboard access to actually flip that toggle; no PR can do it. Until then, if `docker:S8431` is still open on a given PR despite the September 8 fix, mark those findings Won't Fix/False Positive in the dashboard with a reference to it (Docker's own SonarCloud analyzer has no inline suppression mechanism - no `NOSONAR`, since Dockerfile instructions can't carry trailing comments the analyzer reads).
