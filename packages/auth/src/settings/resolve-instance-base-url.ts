...
import { getInstanceUrl } from "./instance-url.js";
 ...
 * injected baseUrl → env `BASE_URL` → DB `instance_url` (via {@link getInstanceUrl}) → dev localhost → throw.
 *
 * Use {@link getInstanceUrl} when you only need the raw persisted setting without env/dev fallbacks.
 */
