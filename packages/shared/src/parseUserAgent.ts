 ...
 * row, or a login's request headers) - isomorphic (no DOM/browser dependency, pure string
 * parsing), so it's the shared source for both apps/admin's own display uses (Sessions list, the
 * Wallet card's "Added from" row) and packages/auth's server-side notification content (a
 * new-location sign-in alert's "device" line) - see packages/auth/src/audit.ts's
 * logLoginNewCountry. Unlike parseDeviceName.ts (apps/admin-only), which reads the CURRENT
 * browser's own navigator.userAgent for self-service device labeling. */
