...
8. Switch to the **Wallets** tab for wallet-adoption analytics: pass adoption and platform mix (Apple, Google, and Samsung Wallet - whichever platforms the event's Wallet settings offer), how many devices or accounts each attendee's pass is actively registered on, adoption by ticket type, cumulative passes issued over time, how long it takes attendees to actually get their pass confirmed installed after the ticket email, wallet lifecycle (how many issued passes are still installed, removed, or never installed at all), and check-in rate compared between attendees with and without a wallet pass.
...
10. Switch to the **Mail** tab for email delivery analytics: delivery-attempt status (queued, accepted, failed, bounced, and so on), how many attendees email actually reached at least once (whether it took one attempt or a resend), the split between first tries and resends, success rate for each email template used, a cumulative chart of successful sends over time, and how many reached attendees went on to open their ticket page online at least once. That last number reflects real ticket-page visits, not whether an email itself was opened - Admitto does not track email opens or link clicks.
