...
1. Open **Events** to see active and archived events in your organisation. Each card shows a map preview when maps are enabled and the event has a pin, a short status (for example **Needs archiving**, **Today**, or **In N days**), location, and attendee count. Archive and restore stay in **Event settings** / organisation Event archiving — not on the card.
