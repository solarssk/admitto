...
6. Use **Notifications** for security alerts to admin staff: a team **webhook** (Discord, Slack, or generic JSON - one URL per organisation, shared by every admin, not per-person), a list of **extra email recipients** who get every enabled alert regardless of their own personal opt-out, and a per-type **on/off** toggle for each alert (repeated failed logins, MFA break-glass use, login/security settings changed, admin login from a new country). Turning a type off only stops active delivery (email, webhook, in-app) - the underlying security event is still recorded either way. **Send test** fires all three channels against the already-saved settings (not unsaved changes): the webhook goes to the shared team destination above, while the test email and in-app notification go only to your own account, not every admin.
