...
6. Use **Notifications** for security alerts to admin staff, split across three cards. Every test action fires the same underlying check against the already-saved settings (not unsaved changes) and shows only the channel(s) it owns.
   - **Webhook**: holds the team destination (Discord, Slack, or generic JSON; one URL per organisation, shared by every admin, not per-person). Its **Send test**, next to Payload format, reports the webhook and in-app results (in-app always goes to your own account; there is no separate in-app card yet, a bell/inbox to view them is a planned future release).
...
7. Use **Security** for MFA, session policy, passkey/security-key sign-in, and trusted third-party script origins (Content-Security-Policy).
   - **Operator "Keep me signed in" duration (days)** sets how long an operator stays signed in after ticking **Keep me signed in** on the sign-in page (3 days by default, 0 to 14). For that session it replaces the operator maximum lifetime and inactivity timeout, so a check-in tablet stays signed in between shifts. Set it to 0 to hide the checkbox. Administrators and Superadmins are not affected and keep their own session limits.
   ...
   - **Session policy**: review or revoke active sessions from **Users & roles** instead.
...
11. Use **Health check** to review Core infrastructure and External integrations status.
    ...
    - **File storage**: reports whether the local upload directory (`UPLOAD_DIR`) exists and is writable. If the folder is missing it shows as degraded (Admitto creates it on the first branding upload). A misconfigured path that points at a file instead of a folder is reported as down.
    - **Run live checks**: an on-demand probe of address lookup, weather, mail transport (SMTP verify / Microsoft Graph token; Power Automate is listed as configured without a live probe), identity providers, Cloudflare Access, or file storage. It also writes and removes a tiny probe file in the upload directory, and can probe the active weather provider when weather is enabled. Expand a row for diagnostics such as latency or endpoint host (no secrets).
