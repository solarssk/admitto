...
6. Use **Security** for MFA, session policy, and trusted third-party script origins (Content-Security-Policy). The trusted-origins list adds an `https://` origin (for example an analytics/monitoring beacon like Cloudflare Web Analytics, or a login challenge widget like Cloudflare Turnstile) to the admin/operator SPA and sign-in pages only; it does not apply to the public ticket page. Review or revoke active sessions from **Users & roles** instead.
