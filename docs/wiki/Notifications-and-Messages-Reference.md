...
| A break-glass CLI command force-resets an account's two-factor or generates an emergency recovery code for it | **Yes, if configured** | The one-time result is still shown only on the operator's own terminal, and the action is recorded in the Security audit log - it also fires the same configured alert as the two rows above |
| A break-glass CLI command bootstraps a new superadmin account | **No** | Recorded in the Security audit log only; not wired to a configured alert |
