...
Add one attendee, find and review attendee records, and make supported attendee-level corrections.
...
### Review or update an attendee

1. Use search and filters to find the attendee.
2. Open the attendee detail page.
3. Review contact details, pass state, delivery activity, check-in activity, notes, and issued items relevant to your task.
4. Make only the supported correction you need, then save it.
5. Confirm the updated value before sending a message or changing admission state.

Use [Importing Attendees](Importing-Attendees) for a prepared list rather than adding many records one by one.

## Expected result

The attendee appears once in the event with accurate contact, ticket, and event-specific details.
...
Saved attendee details become available to templates, ticket rendering, filters, exports, and check-in. A pass-state change can immediately affect whether the ticket can be admitted.
