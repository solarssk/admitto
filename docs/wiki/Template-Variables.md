...
| Event | `event_name`, `event_date`, `event_hours`, `event_location`, `event_address`, `event_map_url`, `google_maps_url`, `apple_maps_url`, `directions_text`, `accessibility_text` | Show event details from the current event. `event_location` is the venue name. `event_date` and `event_hours` display in the event's own regional convention and timezone, the same way they appear on the attendee's ticket - for example `21 August 2026` and `10:00 am - 6:05 pm IST`. `event_hours` shows an open-ended `from`/`until` when only one of start/end is set, and is empty when Event settings has no start/end time set. `event_map_url` inserts a static map image through the image chip; `google_maps_url` and `apple_maps_url` are plain links. Map and link values are empty until the event has a saved location pin. When maps display is disabled for the instance, `event_map_url` stays empty (no broken image). When an admin pastes corrected Maps URLs on the Location tab (**Pin wrong? Fix link**), `google_maps_url` / `apple_maps_url` use those links instead of building from coordinates. |
...
| Wallet | `apple_wallet_url`, `google_wallet_url` | In the body editor, these chips insert an Add to Wallet badge button (badge image linked to the placeholder). Each chip is only offered - and a template can only be saved with it - while that specific platform is turned on in the event's Wallet settings; turning a platform off later blocks re-saving a template that still has its chip until it's removed or the platform is turned back on. Preview and Send test always show a placeholder link, by design, no matter how Wallet is configured. |
