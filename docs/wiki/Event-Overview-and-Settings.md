...
7. Open **Location** to set the venue's full address. Type it in directly, search for it and pick a match, or click the map to drop a pin — dragging the pin or clicking elsewhere adjusts it. Add directions or accessibility notes if attendees need them.
...
- **The General tab's short location field and the Location tab serve different purposes.** The short field is the venue name shown on the events list; the **Location** tab holds the full address, map coordinates, and directions/accessibility notes, and is optional — leave it blank if a map isn't needed for this event.
...
Changes to the event title, date, time zone, location, capacity, or images become the current event configuration. A capacity change can affect future attendee additions and imports; it does not remove existing attendees. The **Location** tab's address, map pin, and directions/accessibility notes are saved with the event; they don't yet appear on tickets or other attendee-facing pages.
...
- **The address search on the Location tab finds nothing, or the map doesn't appear:** try a more specific search (e.g. add a city or country), or drop the pin directly on the map instead. If the map is missing entirely, this instance may have map display turned off; typing the address by hand still works. A banner asking to set up a Support contact is a reminder for a Superadmin to fill it in under Instance settings → General — address search still works either way.
