...
**My account** is split into four tabs, and the active one is reflected in the page's URL so a reload or a bookmarked link returns to the same tab:

| Tab | What's there |
|-----|--------------|
| **Profile** | Display name, regional format, time format, language, phone number, Role, Account type, and the SSO connect/unlink menu (steps 1-11 below) |
| **Password** | Change password, and everything under Two-factor authentication - authenticator app, passkeys, security keys, and backup codes (steps 12-16 below) |
| **Sessions** | Active sessions list and revoke actions (step 17 below) |
| **Notifications** | Your personal alert-type Email/In-app grid (step 18 below) |
...
13. **Two-factor authentication**: click **Set up** to add an authenticator app (TOTP) as a second sign-in factor; this opens in a popup with a QR code to scan and a digit code to confirm. Save the 10 backup codes shown once during setup; they're your fallback if you lose access to the app. Once set up, the button becomes **Manage**, opening a popup with a single **Remove** action that takes out only the authenticator app; your passkeys, security keys, and backup codes are left in place.
    ...
    The options menu (the three-dot icon) in the **Two-factor authentication** card's own header offers two further actions:
    - **Reset everything**: clears every confirmed two-factor method at once (see the dedicated bullet under Important decisions below).
    - **Forget all trusted devices**: ends every "Remember this device" trust for this account at once, the same as a bank's "forget remembered devices" option, without waiting for each one to expire on its own. Available whenever you have a confirmed two-factor method, even without a local password (unlike Reset everything). Disabled whenever there's currently nothing to forget.
...
14. **Passkeys and security keys**: register one or more passkeys or security keys as extra two-factor methods, alongside an authenticator app. Once registered, a passkey or security key can be used the same way a code from your authenticator app can: at the two-factor step after your password (a "Use a passkey or security key" option appears), and to confirm sensitive account actions; see What changes after this action below.
    - If an administrator has turned on passkey sign-in for this instance (Settings → Security), a registered passkey also appears as a **Sign in with a passkey** option directly on the sign-in screen, letting you skip typing your password entirely. Your account's two-factor requirement is already satisfied by the passkey itself in that case, so there's no separate code step afterward.
...
16. **Backup codes**: appears once you have at least one confirmed two-factor method (an authenticator app, a passkey, or a security key).
    - **What it shows**: how many of your 10 codes are still unused, for example "7 of 10 remaining", or **None generated yet** if none have been created for your account. Backup codes are an account-wide fallback: they aren't tied to any single authenticator app, passkey, or security key, and work as a second factor no matter which method you normally sign in with.
    - **How to regenerate**: click **Manage**, then **Regenerate** to invalidate your current codes and get a fresh set of 10, shown once as plain text. Save them somewhere safe, since they can't be viewed again after you close the popup.
    - **What regenerating does**: backup codes can't be removed individually; regenerating is the only way to invalidate old ones.
    - **Step-up requirement**: if your role requires two-factor authentication and you have a confirmed method, regenerating asks you to confirm with a code from your authenticator app, another backup code, or a passkey/security key first.
17. **Active sessions**: review where your account is currently signed in (device, IP, sign-in method, last active) and revoke any session that isn't yours, or all other sessions at once. The list is paginated (10 per page by default; choose 25 or 50 instead). On a phone-width screen it shows as a stack of cards, one per session, instead of a table.
18. **Notifications**: a grid of the security alert types your role can receive (repeated failed logins, emergency two-factor bypass use, login or security settings changed, sign-in from a new country, admin or superadmin role granted), each with its own **Email** and **In-app** switch.
    - Turning a switch off or on saves immediately; there's no separate Save button here.
    - This only controls channels for your own account. The shared team webhook (if your organisation has one configured) is managed separately by a superadmin in Organisation Settings → Notifications.
    - A bell icon in the top-right of every page shows how many of your in-app alerts are unread and opens a short list of the most recent ones (labelled with which organisation each is from, if you have access to more than one). The list shows only the first two lines of each alert, so click one to open a window with its full message, the organisation it came from, and the exact time it arrived (in UTC and in your local time). Opening an unread one also marks it read. Use the double-checkmark icon for **Mark all as read**.
    - The trash icon for **Clear all** permanently deletes your own notification history (asks for confirmation first). This only removes the in-app copy, not your organisation's security audit trail, and future alerts still arrive normally afterward.
    - Anything you never clear yourself is automatically deleted after 30 days.
    - Operator accounts see the same grid and bell, but never actually receive these five alert types (they're aimed at organisation admins and superadmins), so the switches stay effectively unused for that role.
    - Every account, Operators included, always gets one additional, non-configurable alert whenever their own password, two-factor method, passkey, or single sign-on connection changes, whether they made the change themselves or another admin made it for them. It doesn't appear as a row in the grid and can't be turned off, by design: it's the one alert meant to catch an unauthorized change to your own account, so an attacker who already has access can't silence it.
    - Admins and Superadmins also get a personal, non-configurable alert whenever their own account signs in from a country not seen among its recent successful logins, so they learn if their own credentials might be in someone else's hands, not just the rest of the admin team.
    - Anyone with an authenticator app set up gets a third personal, non-configurable alert if a code from that app is submitted again after it was already used to sign in. This is a sign the code may have been seen by someone else, since a genuine authenticator app never produces the same code twice.
...
- An account that requires two-factor authentication (per its role) is asked to confirm with a TOTP code, a backup code, or a registered passkey/security key before a password change, removing the authenticator app, a full 2FA reset, removing a passkey or security key, or regenerating backup codes can complete.
...
- Passkeys and security keys are additional second-factor options alongside the authenticator app, not a replacement for it - your password (or SSO) plus one of these is still how you sign in. The separate "Sign in with a passkey" option on the sign-in screen, when an administrator turns it on, is the only way a passkey replaces typing your password; a security key can't be used there, only for the two-factor step and for confirming sensitive actions. You can register any number of passkeys or security keys, and every one needs a name so you can tell multiple keys apart later.
- An administrator controls passkeys and security keys from Settings → Security, with two switches:
  - **Turn passkeys and security keys off for the whole instance**: a full kill switch. The Add buttons stay visible but disabled (a note above them explains why), and already-registered passkeys and security keys stop working too, both for signing in and for confirming sensitive actions. A user whose only confirmed method is a passkey falls back to a backup code in the meantime.
  - **Turn the sign-in-screen passkey option on or off**, separately from the switch above.
  - The authenticator app is unaffected either way.
- **Unlinking SSO** always demands one proof of identity beyond just being signed in: a TOTP code, backup code, or passkey/security key if you have one confirmed, otherwise your current local password. An account with neither has nothing to unlink with self-service; that's an intentional dead end, not a bug.
  - This proof is required because unlinking replaces the credential that gets you into the account.
...
- A changed display name appears wherever your name is shown to other staff (role assignments, audit/security logs, delivery history).
- A changed password or a session revoke ends every other active session; you'll need to sign in again on those devices.
- After unlinking SSO, Account type switches to **Local account** and sign-in via the removed provider(s) stops working immediately. Use the new local password you set instead.
- After connecting a provider, Account type reflects the new link and you can sign in with either method going forward.
- A newly added passkey or security key is registered immediately and can be used right away: to sign in at the two-factor step, to confirm sensitive account actions, and (a passkey only, when an administrator has turned it on) directly from the sign-in screen. An administrator can turn passkeys and security keys off instance-wide (see Important decisions below), in which case only your authenticator app or a backup code work until it's turned back on.
- Removing your last remaining two-factor method (whether that's the authenticator app via Remove, a passkey, or a security key) means you'll be asked to set one up again the next time you sign in, if your role requires two-factor authentication.
- Regenerating backup codes immediately invalidates every code from the previous batch; only the new set shown at that moment is valid.
...
- **Regenerating backup codes asks for a code:** that's the required step-up check for accounts whose role requires two-factor authentication, not an error. Enter a code from your authenticator app, a saved backup code, or use a registered passkey/security key.
...
- **"Add passkey" or "Add security key" doesn't appear at all:** your account has no local password yet - set one under Password first, or ask an administrator to set one for you.
- **"Add passkey" or "Add security key" is greyed out:** an administrator has turned passkeys and security keys off for this instance; the note above the buttons explains this and who to ask.
- **Removing a passkey or security key asks for a code:** that's the required step-up check for accounts whose role requires two-factor authentication, not an error. Enter a code from your authenticator app, a saved backup code, or confirm with another registered passkey/security key.
...
- **Unlink SSO asks for a password or authenticator code you don't recognize:** that's the required proof of identity, not an error. Enter your current local password, or confirm with a code from your authenticator app, a saved backup code, or a passkey/security key, whichever the dialog is asking for.
...
- **The notification bell never shows any unread count for an Operator account:** the five alert types in the grid are aimed at organisation admins and superadmins, so an Operator is never sent one of those, even with every switch turned on. An Operator's bell still shows an unread count whenever their own password, two-factor method, passkey, or SSO connection changes - that one alert applies to every role and can't be turned off.
