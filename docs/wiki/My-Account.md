...
18. **Notifications**: a grid of the security alert types your role can receive (repeated failed logins, emergency two-factor bypass use, login/security settings changed, sign-in from a new country), each with its own **Email** and **In-app** switch. Turning a switch off or on saves immediately - there's no separate Save button here. This only controls channels for your own account; the shared team webhook (if your organisation has one configured) is managed separately by a superadmin in Organisation Settings → Notifications. A bell icon in the top-right of every page shows how many of your in-app alerts are unread and opens a short list of the most recent ones (labelled with which organisation each is from, if you have access to more than one) - click an unread one to mark it read, or use **Mark all as read** for the whole list at once. Operator accounts see the same grid and bell, but never actually receive these alert types (they're aimed at organisation admins and superadmins), so the switches and bell stay effectively unused for that role.
...
- An administrator can turn passkeys and security keys off for the whole instance, and separately turn the sign-in-screen passkey option on or off, both from Settings → Security. Turning off passkeys and security keys entirely is a full kill switch: the Add buttons stay visible but disabled (a note above them explains why), and already-registered passkeys and security keys stop working too, both for signing in and for confirming sensitive actions - a user whose only confirmed method is a passkey falls back to a backup code in the meantime. The authenticator app is unaffected either way.
- Unlinking SSO always demands one proof of identity beyond just being signed in (a TOTP code, backup code, or passkey/security key if you have one confirmed, otherwise your current local password), precisely because it replaces the credential that gets you into the account. An account with neither has nothing to unlink with self-service; that's an intentional dead end, not a bug.
...
- **The notification bell never shows any unread count:** if your role is Operator, this is expected - the alert types listed on this page are aimed at organisation admins and superadmins, so an Operator account is never actually sent one, even with every switch turned on.
