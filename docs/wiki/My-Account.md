...
- An administrator can turn passkeys and security keys off for the whole instance. There's no toggle for this in Settings yet. It's set in server configuration. Turning it off is a full kill switch: the Add buttons stop appearing, and already-registered passkeys and security keys stop working too, both for signing in and for confirming sensitive actions - a user whose only confirmed method is a passkey falls back to a backup code in the meantime. The authenticator app is unaffected either way.
