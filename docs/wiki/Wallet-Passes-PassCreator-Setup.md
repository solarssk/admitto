...
- **Ticket status** ("Valid" / "Checked in" / "Revoked" / "Cancelled") reflects the attendee's status at the moment the pass was created or last reissued, not a live value - nothing in Admitto refreshes an already-issued pass automatically when the attendee is later checked in or revoked, so a card can keep showing "Valid" after the attendee has actually checked in.
- **Field mapping** also offers this event's own custom fields (Event Settings → Requirements/Registrations) as values - but only ones set up as **Single choice** or **Yes/No**, not free text. A "Yes/No" field shows as "Yes" or "No" on the card; a "Single choice" field shows the option the attendee picked. A custom field mapped here and later deleted, or changed to free text, just stops sending a value - the same as any other unmapped field, not an error.
...
1. **In Admitto**, Event Settings → Wallet's Field mapping supports the same kind of entries as any other field, using placeholders such as "Event type", "Venue room", "Venue entrance" (and door/gate/portal), "Venue phone number", "Venue place ID", and the access-point opening times (venue, doors, gates, box office, parking lots, fan zone). Map any of these to a property name, same as step 4 above. Most of the underlying data comes from Event Settings → General (**Event type**) and → Location's **Venue access details** section.
   - **Venue place ID** has no automatic source, since Admitto's geocoding does not produce Apple Maps' own place identifiers. Look it up yourself in the Apple Maps app (search the venue, tap Share, copy the ID from the link) and enter it in Event Settings → Location.
...
An earlier version of this integration sent a separate `semantics` object through the wallet API, with an event-level "Semantic tags" toggle. That object was never a documented part of PassCreator's pass API and had no effect on issued passes (confirmed by inspecting real `.pkpass` files), so it has been removed with no impact if you had the toggle turned on.
